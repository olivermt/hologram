...
- Bind events with `$` prefix: `$click`, `$click_outside`, `$change`, `$files`, `$submit`, `$blur`, `$focus`, `$key_down`, `$key_up`, `$mouse_move`, `$pointer_down`, `$pointer_up`, `$pointer_move`, `$pointer_cancel`, `$drag_start`, `$drag`, `$drag_enter`, `$drag_over`, `$drag_leave`, `$drag_end`, `$drop`, `$resize`, `$scroll`, `$reach_bottom`, `$reach_left`, `$reach_right`, `$reach_top`, `$select`, `$transition_end`, `$transition_start`, `$transition_run`, `$transition_cancel`. **Not** `phx-click` or `phx-change`.
...
- Use `$files` for native file selection and drop handling. On `<input type="file">` it listens to native `change`; on other elements it listens to native `drop` and Hologram prevents `dragover` default behavior for file drags so the browser will deliver file drops. Use `$drag_over` / `$drag_enter` / `$drag_leave` alongside `$files` only when the UI needs hover feedback or other general drag state. Do not write page-specific JS hooks for basic file upload/drop plumbing.
- `$files` event data is `%{files: files, rejected_files: rejected_files}`. Accepted files are `%{token, name, type, size, last_modified}` maps; `token` points to the browser `File` in Hologram's client-side registry. Rejected files omit `token` and include `reason: :type` or `reason: :size`.
- For file type and size constraints, use `accept` on file inputs or `file_accept` on generic drop zones; use `file_max_size` in bytes on either. Accepted types support exact MIME types (`image/png`), MIME wildcards (`image/*`), and extensions (`.pdf`). Always repeat critical validation server-side.
- Upload a registered file from a client action with `Hologram.Files.upload(token, url, opts) |> Task.await()`. It sends the browser `File` as the request body, defaults to `POST`, uses the file MIME type as `Content-Type` unless `headers` overrides it, and deletes the token after an `ok` response. Pass `cleanup: false` to keep the token for retry or call `Hologram.Files.delete(token)` for explicit cleanup.
- Example file input: `<input type="file" accept="image/png,image/jpeg" file_max_size={10_000_000} $files="upload_files" />`. Example drop zone: `<div file_accept="image/png,image/jpeg" file_max_size={10_000_000} $files="upload_files">Drop files here</div>`. In the action, iterate `params.event.files` and call `Hologram.Files.upload(file.token, "/uploads") |> Task.await()`.

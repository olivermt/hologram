...
- Append `allow_default` so Hologram does not call `preventDefault` for that binding, letting the browser's native default run while the action still dispatches: `$click.allow_default="track"`. Hologram prevents the default on interaction events except keyboard (`$key_down`/`$key_up`) and non-drop-target drag events (`$drag_start`/`$drag`/`$drag_leave`/`$drag_end`), so `allow_default` is meaningful on `$click`/`$submit`/cancelable pointer events and `$drag_enter`/`$drag_over`/`$drop`. Composes with key filters, `stop_propagation`, `.debounce`, and `.throttle`.
